...
const AirportAnalytics = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (activeTab === 'elevation') {
          const response = await axios.get('http://localhost:8080/airports/analytics/avg-elevation');
          setElevationData(response.data);
        } else if (activeTab === 'iata') {
          const response = await axios.get('http://localhost:8080/airports/analytics/no-iata');
          setNoIataData(response.data);
        } else if (activeTab === 'timezone') {
          const response = await axios.get('http://localhost:8080/airports/analytics/common-timezones');
  ...
  const renderElevationChart = () => {
    // Get top 15 countries for better visibility
    const topCountries = elevationData.slice(0, 15);
    ...
    return (
      <div className="no-iata-container">
        <div className="stats-card">
          ...
          <div className="stat-value">{noIataData.count}</div>
