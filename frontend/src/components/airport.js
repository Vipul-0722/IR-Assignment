...
function AirportDataExplorer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post('http://localhost:8080/airports/add', formData);
  ...
  const fetchAirports = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8080/airports/search`, {
        params: {
          query: searchTerm,
          page: currentPage,
          limit: itemsPerPage,
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get('http://localhost:8080/airports/stats');
       ...
       {/* <div style={{display:"flex", justifyContent:"end"}}>
        ...
        <div className="stats-container">
          <div className="stat-card">
            ...
            <p className="stat-value total">{totalAirports}</p>
          </div>
          
          <div className="stat-card">
            <h2>High Elevation Airports</h2>
            <p className="stat-value elevation">{highElevationAirports}</p>
          </div>
          
          <div className="stat-card">
            <h2>Countries Represented</h2>
            <p className="stat-value countries">{countriesRepresented}</p>
          </div>
        </div>
